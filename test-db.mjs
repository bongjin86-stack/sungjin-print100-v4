...
import { loadPricingData, getPaperWeights, getPaperCost, getSizeInfo } from './src/lib/dbService.ts';
...
async function test() {
  ...
  try {
    ...
    data.sizes.forEach(s => {
      console.log(`   - ${s.name} (${s.code}): ${s.width}x${s.height}mm, ${s.up_count}장`);
    });
    console.log('');
    ...
    console.log(`   - 스노우지 120g (467x315): ${snowCost?.cost_per_sheet}원/장, 마진율 ${snowCost?.margin_rate}`);
    console.log(`   - 모조지 80g (467x315): ${mojoCost?.cost_per_sheet}원/장, 마진율 ${mojoCost?.margin_rate}`);
    console.log('');
    ...
    if (a4Info) {
      console.log(`   - A4: ${a4Info.width}x${a4Info.height}mm, ${a4Info.up_count}장`);
    }
    console.log('');
