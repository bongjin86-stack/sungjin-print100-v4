    ...
    <div className="min-h-screen bg-[#f5f7f7]">
      ...
      <main className="max-w-2xl mx-auto px-4 py-8">
        <div className="bg-white rounded-2xl shadow-sm p-6">
          <div className="mb-6">
            <h2 className="text-lg font-semibold text-[#222828]">
              인쇄 파일 업로드
            </h2>
            <p className="text-sm text-[#8a9292] mt-1">
              인쇄에 사용할 파일을 업로드해 주세요
            </p>
          </div>
          ...
          {!uploadedFile ? (
            <div
              className={`relative border-[1.5px] border-dashed rounded-xl p-8 text-center transition-all ${
                isDragging
                  ? "border-[#222828] bg-[#f5f7f7]"
                  : "border-[#cbd0d0] hover:border-[#8a9292] bg-[#f5f7f7]"
            ...
            >
              ...
              {isUploading ? (
                ...
                <>
                  ...
                  <p className="text-[#222828] font-medium mb-1">
                    파일 선택 또는 드래그앤드롭
                  </p>
                  <p className="text-sm text-[#8a9292]">
                    PDF, AI, PSD, EPS, JPG, PNG, TIF, ZIP
                  </p>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-[#222828] truncate">
                      {uploadedFile.displayName}
                    </p>
                    <p className="text-xs text-[#8a9292]">
                      {uploadedFile.displaySize}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  <button
                    ...
                    className="text-sm text-[#222828] hover:underline"
                  >
                    변경
                  </button>
                  <button
                    ...
                    className="w-8 h-8 rounded-full hover:bg-[#d9d9d9] flex items-center justify-center transition-colors"
                  >
                    <svg
                      className="w-4 h-4 text-[#8a9292]"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
          ...
          <div className="mt-6 p-4 bg-[#f5f7f7] border border-[#cbd0d0] rounded-xl">
            <p className="text-sm text-[#222828]">
              30MB 초과 시 이메일로 보내주세요
            </p>
            <a
              href={`mailto:${getEmail()}`}
              className="text-sm text-[#222828] hover:underline mt-1 inline-block"
            >
              {getEmail()}
            </a>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-[#222828] mb-2">
              요청사항{" "}
              <span className="text-[#8a9292] font-normal">(선택)</span>
            </label>
            <textarea
              value={request}
          ...
          {!uploadedFile && (
            <div className="mt-6 text-center">
              <button
                onClick={handleSkip}
                className="text-sm text-[#8a9292] hover:text-[#222828] transition-colors"
              >
                이 단계 건너뛰기
              </button>
              <p className="text-xs text-[#8a9292] mt-1">
                주문 후 이메일로 파일 전송
              </p>
            </div>
          )}
