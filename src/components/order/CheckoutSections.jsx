...
const validateEmail = (email) => {
  ...
  if (!email.includes("@"))
    return "올바른 이메일 형식이 아닙니다. (@를 포함해주세요)";
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
          ...
          <div className="flex gap-2">
            <label className="flex-1">
              <input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    quickPaymentType: e.target.value,
                    quickCost: 0,
                  })
                }
                className="sr-only peer"
            ...
            <label className="flex-1">
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, quickPaymentType: e.target.value })
                }
                className="sr-only peer"
              />
              <div className="py-3 px-4 border border-gray-200 rounded text-center text-sm cursor-pointer transition-all peer-checked:border-gray-900 peer-checked:bg-gray-50 hover:border-gray-300">
            ...
            <div className="p-4 bg-gray-50 rounded-lg text-sm space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="relative flex-1 max-w-[160px]">
                  <input
                    type="number"
                    value={formData.quickCost}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        quickCost: parseInt(e.target.value) || 0,
                      })
                    }
                    placeholder="0"
                    min="0"
        ...
        <div className="pt-2">
          ...
          <div className="p-4 bg-gray-50 rounded-lg text-sm space-y-3">
            ...
            <div className="pt-2 border-t border-gray-200">
              <a
                href="https://naver.me/FG7Ybl82"
                target="_blank"
                rel="noopener noreferrer"
                className="text-[#222828] text-xs underline underline-offset-2"
              >
                네이버 지도에서 보기
              </a>
            </div>
    ...
    <section className="mb-10">
        ...
        {[
          ...
          >
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, taxDocumentType: e.target.value })
              }
            />
      ...
      {formData.taxDocumentType === "tax_invoice" && (
        <div className="mt-4 space-y-4">
          {[
            { key: "taxBusinessNumber", ph: "사업자번호" },
            { key: "taxCompanyName", ph: "상호" },
            { key: "taxCeoName", ph: "대표자명" },
            { key: "taxEmail", ph: "이메일" },
          ].map((f) => (
            <input
              key={f.key}
              type={f.key === "taxEmail" ? "email" : "text"}
      ...
      {formData.taxDocumentType === "cash_receipt" && (
        <div className="mt-4">
          <input
            ...
            onChange={(e) =>
              setFormData({ ...formData, taxPhone: e.target.value })
            }
            placeholder="휴대폰 번호"
            className="w-full px-0 py-3 border-0 border-b border-gray-200 focus:border-gray-900 focus:outline-none transition-colors placeholder-gray-400 text-sm"
          />
