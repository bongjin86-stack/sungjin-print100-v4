...
export default function BindingPage() {
  ...
  const handleAddType = async (e) => {
    e.preventDefault();
    try {
      const { error } = await supabase.from("binding_types").insert({
  ...
  const handleAddCost = async (e) => {
    e.preventDefault();
    try {
      const { error } = await supabase.from("binding_costs").insert({
    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      {(showTypeForm || editingTypeId) && (
        <div className="bg-green-50 border border-green-200 rounded-xl mb-6 p-6">
          <h3 className="font-semibold text-gray-900 mb-4">
            {editingTypeId ? "제본 종류 수정" : "새 제본 종류 추가"}
          </h3>
          <form
      ...
      {(showCostForm || editingCostId) && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl mb-6 p-6">
          <h3 className="font-semibold text-gray-900 mb-4">
            {editingCostId ? "제본 비용 수정" : "새 제본 비용 추가"}
          </h3>
          <form
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setCostFormData({
                    ...costFormData,
                    binding_type_id: e.target.value,
                  })
                }
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3455DB] focus:border-transparent"
                required
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <div className="text-center">
            <p className="text-2xl font-bold text-[#3455DB]">
              {bindingTypes.filter((t) => t.is_active).length}
            </p>
            <p className="text-sm text-gray-500 mt-1">활성 종류</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-[#3455DB]">
              {bindingCosts.filter((c) => c.is_active).length}
            </p>
