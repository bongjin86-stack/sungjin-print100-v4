...
export default function OrderDetailModal({ orderId, onClose, onUpdate }) {
  ...
  const getPaymentMethodLabel = (method) => {
    const labels = { bank_transfer: '무통장입금', card: '카드', toss: '토스', kakao: '카카오페이' };
    return labels[method] || method;
  };

  // 증빙서류 유형 라벨
  const getTaxDocumentTypeLabel = (type) => {
    const labels = { none: '발행안함', tax_invoice: '세금계산서', cash_receipt: '현금영수증' };
    return labels[type] || type;
  };
  ...
  const getFileStatusDisplay = (status, fileUrl) => {
    if (status === 'uploaded' && fileUrl) {
      return (
        <a href={fileUrl} target="_blank" rel="noopener noreferrer" className="text-[#222828] hover:underline">
          다운로드
        </a>
      );
    }
    if (status === 'skipped') return <span className="text-gray-500">이메일 전송 예정</span>;
      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl">
        ...
        <div className="px-6 py-6 overflow-y-auto max-h-[calc(90vh-180px)]">
          ...
          <div className="mb-6 p-4 bg-gray-50 rounded-xl">
            <label className="text-sm font-medium text-gray-700 block mb-2">주문 상태</label>
          ...
          <div className="mb-6">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-2">
              <div className="flex justify-between">
                ...
                <span className="text-sm font-medium text-gray-900">{getDeliveryTypeLabel(order.delivery_type)}</span>
              </div>
              {order.delivery_type !== 'pickup' && (
                <>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-500">수령인</span>
                    <span className="text-sm font-medium text-gray-900">{order.recipient || '-'}</span>
                  </div>
                  ...
                  {order.delivery_note && (
                    <div className="flex justify-between">
                      ...
                      <span className="text-sm font-medium text-gray-900">{order.delivery_note}</span>
                    </div>
                  )}
                </>
              )}
                ...
                <>
                  ...
                  {order.quick_payment_type === 'prepaid' && order.quick_cost > 0 && (
                    <div className="flex justify-between">
                      ...
                      <span className="text-sm font-medium text-gray-900">{formatPrice(order.quick_cost)}</span>
                    </div>
                  )}
                </>
              )}
          ...
          {/* 파일 상태 */}
          <div className="mb-6">
            <h4 className="text-sm font-semibold text-gray-900 mb-3">파일 상태</h4>
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              ...
              {order.file_name && (
                <div className="flex justify-between mt-2">
                  ...
                  <span className="text-sm text-gray-700">{order.file_name}</span>
                </div>
              )}
              {order.request && (
                <div className="mt-3 pt-3 border-t border-gray-100">
                  <span className="text-sm text-gray-500 block mb-1">요청사항</span>
          ...
          {order.tax_document_type !== 'none' && (
            <div className="mb-6">
              <h4 className="text-sm font-semibold text-gray-900 mb-3">증빙서류</h4>
              <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-2">
                <div className="flex justify-between">
                  <span className="text-sm text-gray-500">유형</span>
                  <span className="text-sm font-medium text-gray-900">{getTaxDocumentTypeLabel(order.tax_document_type)}</span>
                </div>
                {order.tax_document_type === 'tax_invoice' && (
                  <>
                    <div className="flex justify-between">
                      <span className="text-sm text-gray-500">사업자번호</span>
                      <span className="text-sm font-medium text-gray-900">{order.tax_business_number || '-'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm text-gray-500">상호</span>
                      <span className="text-sm font-medium text-gray-900">{order.tax_company_name || '-'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm text-gray-500">대표자명</span>
                      <span className="text-sm font-medium text-gray-900">{order.tax_ceo_name || '-'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm text-gray-500">이메일</span>
                      <span className="text-sm font-medium text-gray-900">{order.tax_email || '-'}</span>
                    </div>
                  </>
                )}
                {order.tax_document_type === 'cash_receipt' && (
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm font-medium text-gray-900">{order.tax_phone || '-'}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {/* 송장번호 입력 */}
          {order.delivery_type === 'delivery' && order.status !== 'completed' && order.status !== 'canceled' && (
            <div className="mb-6 p-4 bg-cyan-50 rounded-xl">
              <label className="text-sm font-medium text-gray-700 block mb-2">송장번호 입력</label>
              <div className="flex gap-2">
                <select
              ...
              <p className="text-xs text-gray-500 mt-2">송장번호 입력 시 자동으로 "배송중" 상태로 변경됩니다</p>
            </div>
          )}
          ...
          {order.tracking_number && (
            <div className="mb-6 p-4 bg-cyan-50 rounded-xl">
              <h4 className="text-sm font-semibold text-gray-900 mb-2">배송 추적</h4>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-700">
                  {TRACKING_COMPANIES.find(c => c.value === order.tracking_company)?.label || order.tracking_company} - {order.tracking_number}
                </span>
                <a
                  href={getTrackingUrl(order.tracking_company, order.tracking_number)}
                  target="_blank"
          ...
          <div className="mb-6">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              <div className="text-center mb-4">
                <span className="text-xs text-gray-400 uppercase tracking-wider">ORDER SUMMARY</span>
                <h5 className="text-lg font-semibold text-gray-900 mt-1">{item.productName || '-'}</h5>
              </div>
              <div className="space-y-2 text-sm border-t border-gray-100 pt-4">
                ...
                {item.productionDays && (
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-900">{calculateReleaseDateFrom(order.created_at, item.productionDays)}</span>
                  </div>
                )}
              </div>
              <div className="border-t border-gray-200 mt-4 pt-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-900">{getDeliveryTypeLabel(order.delivery_type)}</span>
                </div>
              </div>
              <div className="border-t border-gray-200 mt-4 pt-4 space-y-2 text-sm">
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-900">{formatPrice(Math.round(order.product_amount * 0.1))}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">배송비</span>
                  <span className="text-gray-900">{order.shipping_cost === 0 ? '무료' : formatPrice(order.shipping_cost)}</span>
                </div>
                {order.quick_cost > 0 && (
                  <div className="flex justify-between">
                ...
                <div className="flex justify-between pt-2 border-t border-gray-100">
                  ...
                  <span className="font-bold text-xl text-gray-900">{formatPrice(order.total_amount)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* 결제 정보 */}
          <div className="mb-6">
            <h4 className="text-sm font-semibold text-gray-900 mb-3">결제 정보</h4>
            <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-2">
              <div className="flex justify-between">
                <span className="text-sm text-gray-500">결제 방법</span>
                <span className="text-sm font-medium text-gray-900">{getPaymentMethodLabel(order.payment_method)}</span>
              </div>
              {order.payment_method === 'bank_transfer' && (
                <>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-500">입금 계좌</span>
                    <span className="text-sm font-medium text-gray-900">{bankInfo.bankName} {bankInfo.bankAccount}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-500">예금주</span>
                    <span className="text-sm font-medium text-gray-900">{bankInfo.bankHolder}</span>
                  </div>
                </>
              )}
            </div>
          ...
          {/* 관리자 메모 */}
          <div className="mb-6">
            <h4 className="text-sm font-semibold text-gray-900 mb-3">관리자 메모</h4>
            <textarea
              value={adminNotes}
