...
export default function PapersPage() {
  ...
  const handleDeletePaper = async (id) => {
    if (
      !window.confirm(
        "이 용지를 삭제하면 관련 단가도 삭제됩니다. 계속하시겠습니까?"
      )
    )
      return;
    try {
  ...
  const resetPaperOrder = async () => {
    if (
      !window.confirm("모든 용지의 순서를 1, 2, 3... 으로 재설정하시겠습니까?")
    )
      return;
  ...
  const startEditCost = (cost) => {
    ...
    setCostFormData({
      paper_id: cost.paper_id,
      weight: cost.weight,
      base_sheet: cost.base_sheet,
      cost_per_sheet: cost.cost_per_sheet,
      margin_rate: cost.margin_rate || "1.5",
    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {(showAddPaper || editingPaperId) && (
          <div className="bg-white rounded-xl border border-gray-200 mb-4 p-6">
            <h3 className="font-semibold text-gray-900 mb-4">
              {editingPaperId ? "용지 수정" : "새 용지 추가"}
            </h3>
            <form
              onSubmit={editingPaperId ? handleUpdatePaper : handleAddPaper}
              className="space-y-4"
            >
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <input
                  type="text"
                  placeholder="코드 (예: artpaper)"
                  value={paperFormData.code}
                  onChange={(e) =>
                    setPaperFormData({ ...paperFormData, code: e.target.value })
                  }
                  className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3455DB] focus:border-transparent"
                  required
                />
                <input
                  type="text"
                  placeholder="이름 (예: 아트지)"
                  value={paperFormData.name}
                  onChange={(e) =>
                    setPaperFormData({ ...paperFormData, name: e.target.value })
                  }
                  className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3455DB] focus:border-transparent"
                  required
                />
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={paperFormData.is_active}
                    onChange={(e) =>
                      setPaperFormData({
                        ...paperFormData,
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-sm">
                  ...
                  {imageFile && (
                    <p className="text-[#3455DB] mt-1">{imageFile.name}</p>
                  )}
                </div>
              </div>
              ...
              <div className="flex gap-2">
                <button
                  type="submit"
                  disabled={uploading}
                  className="px-4 py-2 bg-[#3455DB] hover:bg-[#2a44b0] text-white text-sm font-medium rounded-lg transition-colors disabled:opacity-50"
                >
                  {uploading
                    ? "업로드 중..."
                    : editingPaperId
              ...
              {papers.map((paper, index) => (
                <tr key={paper.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-center">
                    <span
                      className={`px-2 py-1 text-xs rounded-full ${paper.is_active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                    >
                      {paper.is_active ? "활성" : "비활성"}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <button
                      onClick={() => startEditPaper(paper)}
                      className="px-2 py-1 text-xs border border-gray-300 hover:bg-gray-100 rounded-lg transition-colors mr-1"
                    >
                      수정
                    </button>
                    <button
      ...
      <div className="mb-8">
        ...
        {(showAddCost || editingCostId) && (
          <div className="bg-white rounded-xl border border-gray-200 mb-4 p-6">
            ...
            >
              <select
                value={costFormData.paper_id}
                onChange={(e) =>
                  setCostFormData({ ...costFormData, paper_id: e.target.value })
                }
                className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3455DB] focus:border-transparent"
                required
              >
              ...
              <input
                type="number"
                placeholder="평량 (g)"
                value={costFormData.weight}
                onChange={(e) =>
                  setCostFormData({ ...costFormData, weight: e.target.value })
                }
                className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3455DB] focus:border-transparent"
                required
              />
              <select
                value={costFormData.base_sheet}
                onChange={(e) =>
                  setCostFormData({
                    ...costFormData,
                    base_sheet: e.target.value,
                  })
                }
                className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3455DB] focus:border-transparent"
              >
              ...
              <input
                type="number"
                placeholder="장당 단가"
                value={costFormData.cost_per_sheet}
                onChange={(e) =>
                  setCostFormData({
                    ...costFormData,
              ...
              <input
                ...
                step="0.1"
                placeholder="마진율"
                value={costFormData.margin_rate}
                onChange={(e) =>
                  setCostFormData({
                    ...costFormData,
                    margin_rate: e.target.value,
                  })
                }
                className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3455DB] focus:border-transparent"
              />
            ...
            <tbody className="divide-y divide-gray-200">
              {paperCosts.map((cost) => (
                <tr key={cost.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-center">
                    <span
                      className={`px-2 py-1 text-xs rounded-full ${cost.base_sheet === "467x315" ? "bg-gray-100 text-gray-700" : "bg-green-100 text-green-700"}`}
                    >
                      {cost.base_sheet}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right font-medium text-[#3455DB]">
                    {(cost.cost_per_sheet || 0).toLocaleString()}원
                  </td>
                  <td className="px-4 py-3 text-center text-gray-900">
                    {cost.margin_rate || "-"}
                  </td>
                  <td className="px-4 py-3 text-center">
                    <span
                      className={`px-2 py-1 text-xs rounded-full ${cost.is_active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                    >
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <div className="text-center">
            <p className="text-2xl font-bold text-[#3455DB]">
              {paperCosts.filter((c) => c.base_sheet === "467x315").length}
            </p>
            <p className="text-sm text-gray-500 mt-1">467x315</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-[#3455DB]">
              {paperCosts.filter((c) => c.base_sheet === "390x270").length}
            </p>
