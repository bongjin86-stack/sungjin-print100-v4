    ...
    try {
      const { error } = await supabase
        ...
        .insert({
          ...
          setup_cost_double: costFormData.setup_cost_double ? parseInt(costFormData.setup_cost_double) : null,
    ...
    return (
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-500 mt-1">후가공 종류 및 비용을 관리합니다.</p>
        </div>
    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-amber-800">
          <div><strong>per_copy:</strong> 부당 (수량 x 단가)</div>
          <div><strong>per_face:</strong> 면당 (면수 x 단가)</div>
          <div><strong>per_hole:</strong> 구멍당 (구멍수 x 수량 x 단가)</div>
          <div><strong>per_batch:</strong> 100개당</div>
        </div>
      </div>
      ...
      {(showTypeForm || editingTypeId) && (
        <div className="bg-green-50 border border-green-200 rounded-xl mb-6 p-6">
          <h3 className="font-semibold text-gray-900 mb-4">{editingTypeId ? '후가공 종류 수정' : '새 후가공 종류 추가'}</h3>
          <form onSubmit={editingTypeId ? handleUpdateType : handleAddType} className="grid grid-cols-2 md:grid-cols-6 gap-4">
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">단위 타입</label>
              <select
                value={typeFormData.unit_type}
                onChange={(e) => setTypeFormData({...typeFormData, unit_type: e.target.value})}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3455DB] focus:border-transparent"
              >
      ...
      {(showCostForm || editingCostId) && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl mb-6 p-6">
          <h3 className="font-semibold text-gray-900 mb-4">{editingCostId ? '후가공 비용 수정' : '새 후가공 비용 추가'}</h3>
          <form onSubmit={editingCostId ? handleUpdateCost : handleAddCost} className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">후가공 종류 *</label>
              <select
                value={costFormData.finishing_type_id}
                onChange={(e) => setCostFormData({...costFormData, finishing_type_id: e.target.value})}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3455DB] focus:border-transparent"
                required
              >
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">양면 세팅비</label>
              <input
                type="number"
                value={costFormData.setup_cost_double}
                onChange={(e) => setCostFormData({...costFormData, setup_cost_double: e.target.value})}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3455DB] focus:border-transparent"
                placeholder="0"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setCostFormData({...costFormData, cost_per_unit: e.target.value})}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3455DB] focus:border-transparent"
                placeholder="10"
                required
              />
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setCostFormData({...costFormData, unit_type: e.target.value})}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3455DB] focus:border-transparent"
              >
                <option value="per_copy">부당</option>
                <option value="per_face">면당</option>
      ...
      {/* 후가공 종류 테이블 */}
      <div className="mb-8">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">후가공 종류</h2>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">단위 타입</th>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">양면 세팅비</th>
                      <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">단가</th>
                      <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">단위</th>
                      <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">상태</th>
                      <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">작업</th>
                    </tr>
                  </thead>
                    ...
                    {costs.map((cost) => (
                      <tr key={cost.id} className="hover:bg-gray-50">
                        <td className="px-4 py-3 text-center text-gray-900">
                          {(cost.min_qty || 0).toLocaleString()} ~ {cost.max_qty >= 999999 ? '무제한' : (cost.max_qty || 0).toLocaleString()}
                        </td>
                        <td className="px-4 py-3 text-right text-gray-900">
                          {(cost.setup_cost || 0).toLocaleString()}원
                        </td>
                        <td className="px-4 py-3 text-right text-gray-900">
                          {cost.setup_cost_double ? `${cost.setup_cost_double.toLocaleString()}원` : '-'}
                        </td>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <h3 className="text-base font-semibold text-gray-900 mb-4">후가공 데이터 요약</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <div className="text-center">
            <p className="text-2xl font-bold text-[#3455DB]">{finishingTypes.length}</p>
            <p className="text-sm text-gray-500 mt-1">후가공 종류</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-[#3455DB]">{finishingCosts.length}</p>
            <p className="text-sm text-gray-500 mt-1">총 비용 데이터</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-[#3455DB]">{finishingTypes.filter(t => t.is_active).length}</p>
            <p className="text-sm text-gray-500 mt-1">활성 종류</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-[#3455DB]">{finishingCosts.filter(c => c.is_active).length}</p>
