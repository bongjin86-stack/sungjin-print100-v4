...
export function calculatePackaging(quantity, paperThickness, paperWeight, isDoubleSided = true) {
...
export const PAPER_THICKNESS = {
  'snow_100': 0.111, 'snow_120': 0.133, 'snow_150': 0.166, 'snow_180': 0.200,
  'snow_200': 0.222, 'snow_250': 0.278, 'snow_300': 0.333,
  'art_100': 0.095, 'art_120': 0.114, 'art_150': 0.143, 'art_180': 0.171,
  'art_200': 0.190, 'art_250': 0.238, 'art_300': 0.286,
};

export function estimateThickness(weight, paperType = 'snow') {
  const ratio = paperType === 'art' ? 0.00095 : 0.00111;
  return weight * ratio;
}

export function calculateBindingPackaging(quantity, pages, innerWeight, coverWeight) {
  if (!quantity || !pages) {
    return { boxCount: 1, totalWeight: 0, needsFreight: false };
  }

  const sheetArea = 0.06237; // A4 210x297mm
  const innerSheets = Math.ceil(pages / 2);
  const innerWeightKg = (innerSheets * quantity * sheetArea * (innerWeight || 80)) / 1000;
  const coverWeightKg = (quantity * sheetArea * 2 * (coverWeight || 200)) / 1000;
